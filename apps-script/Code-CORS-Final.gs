...
/**
 * HTTP GET リクエスト処理
 * OPTIONS（preflight）リクエストもここで処理
 */
function doGet(e) {
  Logger.log('=== doGet called ===');
  Logger.log('Parameters: ' + JSON.stringify(e.parameter));

  // CORS preflight用のOPTIONSリクエスト対応
  // GETパラメータで method=OPTIONS が渡された場合
  ...
  const output = ContentService
    .createTextOutput(JSON.stringify({
      ...
      version: '3.2',
      deploymentInfo: 'CORS完全対応版（preflight対応）'
    }))
